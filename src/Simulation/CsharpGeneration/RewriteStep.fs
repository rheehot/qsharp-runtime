    ...
    interface IRewriteStep with
        ...
        member this.Transformation (compilation, transformed) = 
            ...
            let targetsQuantumProcessor = 
                ...
                | true, target -> target = "HoneywellProcessor" || target = "IonQProcessor" || target = "QCIProcessor"
                | _ -> false
            let includeReferences = containsEntryPoint && targetsQuantumProcessor

            let allSources = 
                GetSourceFiles.Apply compilation.Namespaces 
                |> Seq.filter (fun fileName -> includeReferences || not ((fileName.Value |> Path.GetFileName).EndsWith ".dll"))
